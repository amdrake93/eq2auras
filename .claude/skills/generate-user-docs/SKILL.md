...
4. **Write** the pages into a local clone of `eq2auras.wiki.git` (`git clone git@github.com:amdrake93/eq2auras.wiki.git` — SSH, not HTTPS; the dev machine authenticates to GitHub over SSH), then **prune orphans** — delete any wiki page **not in the current manifest**. Update `.generated-from` to the `v<VERSION>` + manifest hash, and regenerate `Home.md` + `_Sidebar.md` from the manifest's page list.
