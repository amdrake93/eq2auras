...
{
    [Theory]
    [InlineData("Bolster", "bob", "eq2auras Buffs", "Bolster → Bob")]                        // our single-target: ability → target
    [InlineData("Tortoise Shell", "onlyfans", "eq2auras Buffs", "Onlyfans: Tortoise Shell")] // our group-wide: caster: ability
    [InlineData("Tortoise Shell", "None", "eq2auras Buffs", "Tortoise Shell")]               // defensive: no capture
    [InlineData("Soul Paralysis", "boss", "eq2auras Buffs", "Soul Paralysis")]              // NOT a catalog buff → unchanged
    [InlineData("Bolster", "", "eq2auras Buffs", "Bolster")]                                 // no target → bare
    [InlineData("Bolster", "bob", "", "Bolster")]                                            // a raider's OWN Bolster timer (not our category) → bare, never relabeled
    [InlineData("Holy Shield", "bob", "MyPanelCat", "Holy Shield")]                          // ditto, any non-buff category
    public void Buff_rows_get_two_format_labels_only_in_our_category(string name, string combatant, string category, string expected)
        => Assert.Equal(expected, TimerListBuilder.Label(name, combatant, category));
