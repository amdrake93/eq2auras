...
- **A separate reviewer agent reviews every written artifact** (code rounds on request) — run via the `requesting-design-review` skill, which carries the full writer/reviewer/loop contract. The automated review loop runs only on Alex's explicit per-artifact invocation; the loop then counts as one authorized turn. In short: process pasted feedback with `superpowers:receiving-code-review`; record plan-watch items in the backlog and "no change" decisions in a doc (the reviewer can't see this conversation and re-raises anything unrecorded).
