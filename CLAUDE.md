...
- **A separate reviewer agent reviews every design artifact** (spec, plan, or fix-flow change) — run via the `requesting-design-review` skill (user-scoped at `~/.claude/skills/requesting-design-review/`, not in this repo), which carries the writer/reviewer/loop contract. The automated review loop runs only on Alex's explicit per-artifact invocation; the loop then counts as one authorized turn. In short: process pasted feedback with `superpowers:receiving-code-review`; record plan-watch items in the backlog and "no change" decisions in a doc (the reviewer can't see this conversation and re-raises anything unrecorded).
