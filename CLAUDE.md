...
- **A separate reviewer agent (another Claude session) reviews every artifact** — the full contract is `docs/review-workflow.md`. In short: process pasted feedback with `superpowers:receiving-code-review`; record plan-watch items in the backlog and "no change" decisions in a doc (the reviewer can't see this conversation and re-raises anything unrecorded).
