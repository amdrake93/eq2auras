...
**Standing live-test item:** the merge-gate script below is runnable solo (custom trigger), but the behavior that matters — LATE clearing instantly on real recasts under raid-scale instance stacking — only shows for real at the **Sunday 2026-07-12 raid**. Capture spike JSONLs that night (schema now carries `master` + `instances`, so the logs self-verify: every `notify`-while-LATE incident should show governance flipping to the new master the same tick) and ferry them to `spike-data/2026-07-12/` for the verification pass.
