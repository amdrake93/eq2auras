...
**Reviewer-mandated plan requirements** (3rd-party spec review, 2026-07-02 — the plan review will check these): hoist `PaletteAssigner` out of the tracker into `OverlayEngine` (one shared map); fix the tracker's **in-place `FillArgb` mutation** before dual-routing (same reading objects to two trackers → second resolves from the first's output — copy per group or resolve non-mutating); lock `SettingsStore` around mutate-and-save (tab knobs on ACT UI thread vs drag-end on overlay STA thread); nullable position fields (DCJS zero-trap) + 0-value enum defaults; bidirectional legacy migration (Panel A seeded, Panel B defaults, save mirrors A to top-level); `OverlayEngine` in Core TDD'd on Mac, plugin stays instantiator/supplier (no async, no non-GAC field types); end with a concrete live-verification script (A-only/B-only/both/neither routing, drag all four + restart persistence, dual-flagged color identity across differing `ColorSource`).
