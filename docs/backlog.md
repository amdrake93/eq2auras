...
### NEXT UP — slice 6: element dimensions — SPEC'D 2026-07-02, review pending
Field verdict on slice 5's resize grip: unintuitive, "like trying to guess dimensions" — retired. Settled model (SPEC §Element dimensions): **drag is for position, numbers are for size** (the WeakAuras division). Per-group numeric knobs `RowWidth`×`RowHeight` (defaults 250×26; text-fit floor on height) and `RadialSize` (diameter, default 110; oval = real elliptical-wedge work, deferred until someone asks; LATE card derives ~1.55×). Tab numeric fields, live-apply via the existing rebuild-once path; `ListScale`/`CenterScale` retired (unstable-only, stale keys ignored). Slice 5's palette/font knobs stand; their remaining live verification folds into slice 6's script.
