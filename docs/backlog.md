...
- Phase-1 odds → SHIPPED (v0.1.91, 2026-07-09; casual live verify during normal play): **Debug mode** (tab checkbox, default off = lifecycle events only; on = full per-tick dump) + **rolling log retention** (14 days / 200 MB at load; debug mode on suppresses the sweep). Feature enable/disable **dropped** — ACT's own plugin checkbox is that switch (Alex). Plan: docs/plans/2026-07-09-debug-mode-log-retention.md.
