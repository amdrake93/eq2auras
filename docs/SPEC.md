...
- List orientation and sort (soonest-to-expire) — window positions and element dimensions are Settings knobs now (§Moving the overlay, §Element dimensions).
...
- Placement-grid pitch and line colors (§Moving the overlay).
...
- **Current knobs (per group):** `ColorSource` — `Palette (default) | Greyscale | ActColor`; `EscalationStyle` — `CenterRadial (default) | HighlightInPlace`; font family + base size (§Typography); element dimensions (`RowWidth`/`RowHeight`/`RadialSize` — §Element dimensions); the four window positions (set by dragging — §Moving the overlay). **Global knobs:** the color palette (`Settings.PaletteArgb` — §Timer colors). Everything still listed under *Baked-in constants* is a future knob awaiting promotion into `Settings`.
...
- **Unlock also shows a full-screen placement grid** — a reference for positioning, pinned to the primary monitor's bounds and **permanently click-through** (it cannot be moved because it is not movable: no chrome, no drag handling, `WS_EX_TRANSPARENT` set once and never toggled). Major lines every **1 logical cm**, fainter minor lines every 0.5cm — "logical": 1 cm = 96/2.54 ≈ 37.8 DIPs, the same coordinate space as the window positions, exactly physical only when display scaling matches the panel's true density. Lines are 1 DIP, aliased (device-pixel-exact only at 100% scaling — consistent with the Phase-1 DPI stance). **The grid sits beneath the overlay windows**, kept there deterministically: when the grid shows, each overlay window re-asserts its topmost position above it, and being click-through the grid never activates and never re-raises — the windows being positioned always draw above their reference. (Other applications' topmost windows may sit below the grid while move mode is open; only this plugin's windows are lifted.) Drawn once (no per-tick work); appears with move mode, disappears on re-lock; no persisted state. Grid pitch and line colors are Phase-1 constants (§Baked-in constants). The grid lives with the reusable overlay-framework pieces (`ClickThrough`, `MoveChrome`) — shared plumbing for every future feature module in this one plugin.
...
- Dimensions are **tab knobs** (numeric fields in each panel's group box), clamped to shared `Settings` constants — row width 100–800, row height 16–100, radial size 40–400 — live-applied through the same rebuild-once path as the font knob, persisted like every other knob. Text sizes never change with element dimensions.
...
Each panel carries a font knob applying to both of its windows: `FontFamily` (any installed Windows font, by name; null = system default — today's look) and `FontBaseSize` (in WPF device-independent pixels; null = 13 — today's look. The native font picker speaks *points* — the surface converts, `points × 96 / 72`). The overlay's text roles derive **proportionally** from the base — row text = base, pie seconds ≈ 2.6×, the LATE tag ≈ 1.7× — so a size change never breaks the visual hierarchy. Per-role customization is a later phase (trajectory: per-window, then per-element, as customization abstracts). Tab surface: a "Font…" button in each panel's group box opening the native `FontDialog` (family + size in one dialog), with a label showing the current choice. Font and element dimensions (§Element dimensions) are deliberately orthogonal: **dimensions set how much space an element takes; font sets how readable its text is** — text never changes with element dimensions.
...
**Build & publish (CI):** a GitHub Actions `windows-latest` workflow, on push, runs **`msbuild`** (not `dotnet build` — WPF's XAML compile requires MSBuild) and publishes the built plugin as the asset of a rolling prerelease. The **version/tag is stamped by CI** (e.g. the run number or `git describe`), never by hand or from wall-clock in code. Branch pushes run the same workflow **verify-only** (tests + compile + artifact, no publish); publishing is a `main`-only event.

**README status section (CI-stamped).** The README carries a Status block: GitHub's native Actions build badge, version and release-date pills, and quick links (dev-latest release page, CI runs, SPEC, backlog). Everything between `<!-- status:begin -->` and `<!-- status:end -->` is **machine-owned** — after publishing, the main build rewrites the block with the just-released version and UTC date and commits it back with `[skip ci]` (consequence: one bot commit on `main` per release; pull before pushing). The version/date pills are **shields.io *static* badges** — the data is baked into the badge URL, which is the only badge form that works here: shields.io cannot read a private repo, while the Actions build badge renders because anyone who can see the README has repo access. Failure mode is benign (a stale block, corrected by the next release).
