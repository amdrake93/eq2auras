...
- Dimensions are **tab knobs** (numeric fields in each panel's group box), clamped to sane bounds (constants), live-applied through the same rebuild-once path as the font knob, persisted like every other knob. Text sizes never change with element dimensions.
