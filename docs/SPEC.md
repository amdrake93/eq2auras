...
- **`RowSpacing`** (null = 4, today's look): the vertical gap between rows, in **raw DIPs — flat, never derived or scaled** (the old `4 × height-ratio` derivation is retired; the configured value wins). `0` = rows touching; rows never overlap. Clamped 0–50.
