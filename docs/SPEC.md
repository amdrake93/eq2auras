...
**Displayed combatants:** the encounter's **allies** (`EncounterData.GetAllies()` — ACT's own ally classification), sorted descending by the primary metric with a deterministic name tie-break (never Details' epsilon-seeding wart). **Every ally travels in the frame** — visibility is the window's scrolling concern (§The meter window), never the data's. Percent = the combatant's share of all allies' total for the primary metric; the header total is that same all-allies sum — both whole-raid regardless of what's visible.
