...
**Displayed combatants — mirror ACT's mini parse.** The row set is **every combatant in the encounter** (`EncounterData.Items`), filtered by ACT's own `ShowOnlyAllies` rule — which in ACT is a per-export *option*; we bake in ACT's **shipped default (`ShowOnlyAllies: true`)**, so it is always on here, not a meter knob. Under it, a combatant is hidden only when it is *not* in `GetAllies()` **and** `GetAllies()` is non-empty — ACT's escape hatch. `GetAllies()` is you-relative (seeded from `charName`, grown by shared-combat edges), so before the player acts it is **empty**, the filter switches off, and every combatant shows — the fix for the field case where a groupmate is dealing damage but the player hasn't engaged (it also, faithfully, shows the enemy mob in that window). Both resolve the instant the player engages: their combatant appears, the ally graph links, non-allies filter out. This deliberately mirrors the mini parse's **default-on** skip rules — the `ShowOnlyAllies` filter and the `"Unknown"` drop; ACT's two other skip clauses (the export-filter-space and selective-parsing options) are off by default and not reproduced. Warts included: there is no clean friend/foe flag in ACT (`GetCombatantType()` itself routes through `GetAllies()`), so inventing a classifier was rejected. The adapter snapshots every combatant with an `IsAlly` flag (its `GetAllies()` membership); the filter itself lives in Core (testable). `"Unknown"` (the parser's unsourced-hit attacker) is always dropped. Rows sort descending by the primary metric with a deterministic name tie-break (never Details' epsilon-seeding wart); **every surviving row travels in the frame** — trimming to the visible count is the window's scrolling concern (§The meter window), never the data's. Percent = share of the **displayed** set's total; the header total is that same sum.
...
- **locked** — geometry frozen. **Unlocked**, a window is **moved by dragging its header** and **resized by dragging its right and bottom edges** — the right edge sets the width, the bottom edge sets how many rows are visible (snap to whole rows, §Configuration); repositioning is the header's job, so the top-left stays anchored while resizing. **Locked** freezes both position and size; **lock freezes geometry only** — the menu, scrolling, the settings window, and future row clicks keep working. Toggled per meter window from its own right-click menu (default unlocked), persisted. The timer module's global move-mode checkbox does not govern meter windows: their interactivity makes a separate unlock mode unnecessary (move mode's bundled unlock exists precisely because timer windows are click-through). Edge-resize is a **meter-only** capability — timer windows are sized by numeric tab knobs — so by the convergence guardrail's ceiling bracket it lives in the meter window, not the shared base.
- **interactive content** — the meter window's content surface accepts clicks (menu now; row drill-down later).

**Header (the interaction surface).** A persistent header strip: **`(duration) title — metric` on the left, the metric's total on the right**, plus a **⚙ cog** that opens the window's settings (§Configuration). The header is the **drag handle** (drag-end persists position, same crash-safe pattern as timer windows), and **right-click anywhere on the window opens the menu** (§Configuration). The title is the encounter's **strongest-enemy-so-far**, computed per poll (`GetStrongestEnemy(charName)` — the same call ACT's own combat-end finalize makes; ACT only assigns `Title` at combat end); it can flip mid-fight as damage shifts between mobs — **known, accepted behavior**, not a bug. The **title trims to an ellipsis** so the metric label and total stay visible — EQ2 mob names ("a lamia deathcaller") are long, and duration/metric/total are the fixed anatomy that must never be pushed off or overlapped. Richer header stats are a later slice.

**Configuration — the right-click menu and the settings window** (Details'-faithful split). **Right-click** anywhere on a window opens the **menu**: the **metric picker** (DPS / HPS / Cures), **Lock window**, **New meter window**, and **Close this window** (disabled when only one window remains). The header's **⚙ cog** opens the window's **settings window** — per-window, modeless, and **live-applying**: **row height**, **font** (the native picker, stored in DIPs, as the timer tab does), a single **opacity** knob that scales the fill and the backplate together, and **Reset to defaults**. Metric and lock live on the menu and are not duplicated in the settings window. A window's height is the header plus the visible-row count times the row height, so the row-height knob thickens the rows while the window re-fits to keep the same number of rows; **width** is set by resizing, not a knob; row **spacing** is fixed at zero (the meter wants none) and is deliberately not a knob. The menu is styled to the overlay's dark aesthetic and the settings window is a dark overlay-styled window — no raw ACT chrome.

**Multiple windows.** Each window carries its own `MeterWindowConfig` (metric, position, size, lock, appearance knobs — §Settings); windows share nothing but the data and **one ally-color map** (an ally reads the same color in every window). The tab's **Meter enabled** checkbox is a **show/hide toggle over the persisted window list**: disabling closes the windows but keeps their configs, enabling restores exactly the windows you had, and the first-ever enable (empty list) seeds one default DPS window. **New meter window** opens a **default** window (DPS, unlocked, default appearance) at a cascade offset from the window it was invoked from — it deliberately does **not** inherit the source's config (a cloned *locked* window opened on top of its source and couldn't be dragged clear); you then re-point it at another metric from its own menu (this is how a DPS window and an HPS window come to sit side by side). You cannot close the last window — the tab toggle is the master off-switch.

**Rows.** The settled layout (mocked and picked, 2026-07-15): name on the left, value + percent on the right, both overlaid on a colored proportional fill — the house style, matching timer rows; **no rank number**. Row color reuses the timer module's palette *mechanism* — first-fired-order, name-keyed assignment over the global `Settings.PaletteArgb` knob — but through the **meter's own assignment map**, keyed by ally name and **shared across every meter window** (an ally keeps one color across fights and across windows for the plugin-instance lifetime, same stability rule as timer colors). Deliberately **not** the timer module's map instance: ability names and ally names are disjoint namespaces with no cross-module color meaning, and sharing one first-fired sequence would let a raid's worth of ally names shift which slots the shipped timer colors land on. Part II §Timer colors stays true as written — its map remains fed by timer names only.

**Meter display defaults are for reading, not glancing-past.** A timer overlay is deliberately translucent — it sits over the playfield and must not block it. A meter is the opposite: you park it where you *read* it, so it renders **near-opaque** (opaque row backplate, vivid fill) with **readable light value text** and **zero row spacing** (rows touch, Details-style). These differ from the timer's translucent look, so where they live in the **shared** bar primitive they are **parameterized** — the fill's opacity is a construction parameter defaulting to the timer's value, and the trailing-text color defaults to readable-light (which the timer already overrides per-urgency every tick) — never a blanket change that would regress the timer's field-tuned appearance. **Opacity and row height are per-window knobs** (the settings window): the meter feeds its configured opacity into that same construction parameter, so the two baked constants become live per-window values with the timer's defaults untouched. Row **width** is set by resizing the window; row **spacing** stays fixed at zero — the meter wants none, so it is deliberately not a knob.
...
- **Plugin (net472/WPF):** the **encounter adapter** — the per-poll ACT read (lock → snapshot allies' totals + the encounter's title/times into Core DTOs → release) — and the meter windows (header, menu, settings window, row visuals) on the shared base. The host holds the live list of windows and, each poll, fans the one shared snapshot to each window's metric through the one shared `MeterEngine`/palette.

**Poll cadence:** the meter samples on the plugin's existing 100 ms poll tick at a divider (baked constant, every Nth tick, effective ~2–4 Hz — Details' refresh-class cadence; the mini parse's default is every 5 *seconds*). The snapshot cost stays in the engine doc's safe zone by construction: per-combatant **total** properties (the incremental-cached kind) plus a small fixed set of encounter-level per-call loops — the ally list, `GetStrongestEnemy()` for the title, `StartTime` (live branch), encounter `Duration` (frozen branch) — the cost class `docs/act-parse-engine.md` §Performance rates "fine at 1–10 Hz". The expensive shapes (per-combatant multi-segment `Duration`, `EncId`/hashing) are never read.

**Settings:** `MeterSettings { enabled; windows[] }` on the global `Settings` — a per-window `MeterWindowConfig` list (DCJS rules: nullable positions/dimensions — null, never zero, means "unset, use the default", per the timer windows' convention; enum/bool defaults at the 0-value). Each config persists its **metric key** (missing or unknown → the DPS default, the registry lookup being the forward-compat guard), **position**, **size** (width and the visible-row count), **locked** flag, and appearance knobs (**row height, font, opacity**). `enabled` defaults **off** (opt-in) and is a show/hide toggle over the window list, which persists independently; a legacy single-window `MeterSettings` migrates once into a one-element list.
...
- **Slice 1:** one meter window · current segment · allies · metric picker over {DPS, HPS, Cures} · the `(duration) title — metric | total` header · overlaid rows · width-lerp animation · wheel-scrolled rank window · lock · the two shared-substrate extractions + timer re-seat.
- **Slice 2 (this design):** the per-window configuration surface — right-click menu (metric · lock · new/close window) + a ⚙ settings window (row height · font · opacity) · multiple windows over one pipeline with a shared ally-color map · edge-resize (width + visible-row count) · the shared per-window `MeterWindowConfig` model.
- **Deferred to later slices:** secondary-data-point selection UX (multi-value row text — guild-requested, next in line); segment picker (history/overall); richer header stats; actor-filter modes; owner-merged pets; per-window click-through-opt-in knob; row drill-down; skins/chrome; user-defined metrics (declarative, then scripted); rolling DPS; boss-only overall policies; row-color-by-class (pending an investigation of what class data EQ2/ACT exposes).
