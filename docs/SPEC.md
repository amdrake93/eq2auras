...
- **`RowWidth` × `RowHeight`** (null = 250 × 26, today's look). Width-flavored geometry (fill drain math) derives from width; height-flavored geometry (corner radii, margins, spark thickness) derives from `RowHeight / 26`. **Row height floors at text-fit** (the row's own text height + padding) regardless of the configured value — rows can never be smaller than their text; denser lists come from lowering the font size first (§Typography).
