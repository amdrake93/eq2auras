...
**Displayed combatants — mirror ACT's mini parse.** The row set is **every combatant in the encounter** (`EncounterData.Items`), filtered by ACT's own `ShowOnlyAllies` rule: a combatant is hidden only when it is *not* in `GetAllies()` **and** `GetAllies()` is non-empty — ACT's escape hatch. `GetAllies()` is you-relative (seeded from `charName`, grown by shared-combat edges), so before the player acts it is **empty**, the filter switches off, and every combatant shows — the fix for the field case where a groupmate is dealing damage but the player hasn't engaged (it also, faithfully, shows the enemy mob in that window). Both resolve the instant the player engages: their combatant appears, the ally graph links, non-allies filter out. This is a deliberate **1:1 with the mini parse**, warts included — there is no clean friend/foe flag in ACT (`GetCombatantType()` itself routes through `GetAllies()`), so inventing a classifier was rejected. The adapter snapshots every combatant with an `IsAlly` flag (its `GetAllies()` membership); the filter itself lives in Core (testable). `"Unknown"` (the parser's unsourced-hit attacker) is always dropped. Rows sort descending by the primary metric with a deterministic name tie-break (never Details' epsilon-seeding wart); **every surviving row travels in the frame** — trimming to the visible count is the window's scrolling concern (§The meter window), never the data's. Percent = share of the **displayed** set's total; the header total is that same sum.
