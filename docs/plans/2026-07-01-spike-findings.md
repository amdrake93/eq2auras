...
- [~] **Reload verdict (Task 8) — PARTIAL, previous "restart required" conclusion was premature.** What we actually observed: **Browse-adding the plugin a second time** is rejected ("duplicate plugin found, file already loaded into memory, restart ACT") — that's ACT refusing a duplicate *list entry*, a different code path from reloading. The real experiment — **overwrite the DLL in place, then toggle the EXISTING entry's Enabled checkbox** (the ACT_Adder / EQAditu-updater pattern) — was **not run**. Byte-loading (confirmed via the AssemblyResolve finding) means the file isn't locked and `Assembly.Load(byte[])` can bring in fresh bytes on re-enable, so live reload may work after all. **Decisive test queued: orange build.** Sub-questions it also answers: (a) is the DLL overwritable while the plugin is enabled (file-lock), (b) does the toggle run new bytes (box colour), (c) does an unchanged-on-disk `Core.dll` still resolve across the reload.
