...
- [ ] **Step 2: Wire the label + host the third group (transcribe-only).** Route the row visual's displayed text through `TimerListBuilder.Label(reading.Name, reading.Combatant)`. `OverlayHost.Start` **already** iterates `_settings.Panels.Count` with a per-index default position (`OverlayHost.cs:48-51,502`), so hosting the third group needs no loop change — what *does* need attention: **`PanelNames`** has only two entries (`OverlayHost.cs:19`), so the buff group falls back to "Panel 3" — add a third name ("Buffs"); and the settings tab builds group boxes only for Panels A/B (`Eq2AurasPlugin.cs:115-116`). **v1 decision:** the buff window uses **default knobs** — **no** dedicated group box this slice; a "Buffs" group box rides the timer-config tab-redesign arc (a third box on the already-cramped tab now is throwaway work). So this task's only tab change is the `PanelNames` third entry.
