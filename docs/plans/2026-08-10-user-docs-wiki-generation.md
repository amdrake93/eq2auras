...
**Both the manifest sources above and the EXCLUDE list below name header *prefixes*** (a name resolves to the SPEC section whose `###` header begins with it — see Step 2's run-process **item 3** (section resolution by header prefix)). Sub-features live *inside* their section, not as separate entries: `§The meter window` covers the multiple-windows / right-click-menu / ⚙-settings / row-drill-down surfaces, and `§Segments mirror ACT's encounter list` covers the segment picker.
...
- **Plan-watch item (from spec review, carried):** the promoted-`vX.Y.Z`-commit source read is **Task 1 Step 2** (`SKILL.md`) run-process **item 1**; the shipped-only manifest scope is the **Step 3** page-manifest (its EXCLUDE list + the default-deny rule) plus the absent-section rule in **Step 2** run-process **item 3**; the incremental release-diff targeting (churn mitigation) is **Step 2** run-process **item 2**. Plan review verifies each is concrete.
